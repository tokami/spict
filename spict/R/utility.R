...
#' @name retape.spict
#' @title Retape spictcls object based on updated inp list
#'
#' @param rep A result report as generated by running \code{fit.spict}.
#' @param inp Input list
...
#'     (default: 0).
#'
#' @return Retaped rep object
retape.spict <- function(rep, inp, dbg = 0){
    repin <- rep
    inpin <- inp
    ...
    sdfailflag <- class(repout) == 'try-error'
    if(sdfailflag){
        warning('Could not calculate sdreport.\n')
        repout <- NULL
    }
    if(is.null(repout)){ # If sdreport failed or was not calculated
        repout <- list()
        if (sdfailflag){
            repout <- list()
            repout$sderr <- 1
            repout$par.fixed <- repin$opt$par
            repout$cov.fixed <- matrix(NA, length(repin$opt$par), length(repin$opt$par))
        }
    }
    repout$inp <- inpin
    repout$obj <- objt
    ...
    if(!sdfailflag && inpin$reportall){
        if (!inpin$osar.method == 'none'){
            reposar <- try(calc.osa.resid(repout))
