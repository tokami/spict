...
#' @name process.resid
#' @title Estimate process residuals
#'
#' @param rep A result report as generated by running \code{fit.spict}.
#'
...
process.resid <- function(rep){
    ...
    rep$obj$env$data$resFlag <- 1
    rep$obj$retape()
    sdrep <- sdreport(rep$obj,rep$opt$par)

    ## Biomass
    idx <- which(names(sdrep$value)=="resB")
    resB <- rmvnorm(1,mu=sdrep$value[idx], Sigma=sdrep$cov[idx,idx])

    ## Fishing mortality
    idx <- which(names(sdrep$value)=="resF")
    resF <- rmvnorm(1,mu=sdrep$value[idx], Sigma=sdrep$cov[idx,idx])

    res <- as.data.frame(cbind(time = inp$time[-length(inp$time)],
                               resid.B = resB,
                               resid.F = resF))
