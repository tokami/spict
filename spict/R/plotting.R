...
#' @param add.loess Add Loess line to residuals
#' @return Nothing.
#' @examples
#' data(pol)
#' rep <- fit.spict(pol$albacore)
...
#' plotspict.osar(rep)
#' @export
plotspict.osar <- function(rep, collapse.I=TRUE, qlegend=TRUE, add.loess = FALSE){
    check.rep(rep)
...
plotspict.diagnostic <- function(rep, lag.max=4, qlegend=TRUE, plot.data=TRUE, mfcol=FALSE,
                                 stamp=get.version(), add.loess=FALSE){
    repflag <- FALSE
    mar <- c(4.7, 4.1, 2.5, 2)
    #op <- par()
    ...
    # OSAR plots
    if ('osar' %in% names(rep)){
        plotspict.osar(rep, collapse.I=FALSE, qlegend=qlegend, add.loess=add.loess)
        # Catch ACF
...
#'
#' @export
plot.spictcls <- function(x, stamp=get.version(), verbose=TRUE, CI = 0.95, add.loess=FALSE,...){
    check.rep(x)
    rep <- x
    ...
    if ('par.fixed' %in% names(rep) & rep$inp$do.sd.report){
        ...
        if ('osar' %in% names(rep)){
            plotspict.osar(rep, qlegend=FALSE, add.loess=add.loess)
        }
        if (inp$reportall){
            if ('infl' %in% names(rep)){
...
plotspict.diagnostic.process <- function(rep, lag.max=4, qlegend=TRUE, plot.data=TRUE, mfcol=FALSE,
    ...
    abline(h=0, lty=3)
    if(add.loess){
        mod <- loess(B ~ time, data=rep$process.resid)
    ...
    abline(h=0, lty=3)
    if(add.loess){
        mod <- loess(F ~ time, data=rep$process.resid)
        j <- order(rep$process.resid$time)
        lines(rep$process.resid$time[j], mod$fitted[j], lwd=1.5)
    }
