...
plotspict.seasonP <- function(rep, stamp=get.version()){
    ...
    if (!'sderr' %in% names(rep) & 'logSdSP' %in% names(rep$par.fixed)){
        ...
        logSdSP <- get.par('logSdSP', rep)
        naflag <- any(is.na(logSdSP[, 1]))
        
        inds <- which(colnames(rep$cov.fixed) == 'logSdSP')
##        ssf <- get.par('seasonsplinefineP', rep)
        covmat <- rep$cov.fixed[inds, inds]
        if (rep$seasonalProd==2){
        logSPvec <- rep$inp$ini$logSPvec
        nSP <- length(logSPvec)

        ## example forced seasonal pattern (but how to use sdSP?)
        logSPvec <- sin(1:(1/rep$inp$dteuler))
