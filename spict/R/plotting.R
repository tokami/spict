...
#' @return Nothing.
#' @export
plotspict.seaprod <- function(rep, stamp=get.version()){
    if (!'par.fixed' %in% names(rep)){
        stop('Input object was not a valid output from fit.spict()!')
    }
    if (!'sderr' %in% names(rep) & 'logphiSP' %in% names(rep$par.fixed)){
        ...
        if (!naflag){
            lines(t, yl, col=cicol3)
            lines(t, yu, col=cicol3)
        }
        lab <- strftime(c(jan, apr, jul, oct, jan), format='%b')
        ats <- c(0, 0.25, 0.5, 0.75, 1)
        abline(v=ats, lty=3, col='lightgray')
        ats2 <- pretty(c(yl, yu))
        if ("true" %in% names(rep$inp)){
            ats2 <- pretty(c(yl, yu, ytrue))
        }
        abline(h=ats2, lty=3, col='lightgray')
        lines(t, y, lwd=1.5, col=4, typ='s')
        if ("true" %in% names(rep$inp)){
            lines(ttrue, ytrue, lwd=1, col=true.col(), typ='s')
        }
        axis(1, at=ats, labels=lab)
...
#' @name plotspict.hcr
#' @title Plot harvest control rules
#' @details Plots harvest control rules
#' @param rep A result report as generated by running fit.spict.
#' @param stamp Stamp plot with this character string.
#' @return Nothing.
#' @export
#'
plotspict.hcr <- function(rep,
                         quant = "BBl",
                         hcrs = c("f0","dl","2/3"),
                         stamp=get.version()){
    ...
    ## quantities
    BBl <- get.par("logBBl",rep, exp = TRUE)
    time <- inp$time
    Tman <- time[inp$indmanstart]
    Tpred <- tail(time[inp$indpred],1)
    timepred <- time[inp$indmanstart:inp$indmanend]
    
    time_nopred <- c(time[!(time %in% timepred)],timepred[1])
    BBl_nopred <- BBl[time %in% time_nopred,]

    ## settings
    cicol2 <- rgb(0, 0, 1, 0.1)
    cicol3 <- rgb(t(col2rgb("green"))/255, alpha = 0.1)
    cicol4 <- rgb(t(col2rgb("red"))/255, alpha = 0.1)
    cicol5 <- rgb(t(col2rgb("orange"))/255, alpha = 0.1)
    ylim <- range(as.numeric(BBl[,1:3]))
    
    ## F = 0    
    if("f0" %in% hcrs){
        inpt <- make.ffacvec(inp, 1e-8)
        rept <- rep
        rept$obj$env$data$ffacvec <- inpt$ffacvec
        rept$obj$env$data$MSEmode <- 2
        rept$obj$retape()
        rept$obj$fn(rept$opt$par)
        sdr <- sdreport(rept$obj)
        BBl_f0 <- get.par("logBBl",sdr, exp = TRUE)[inp$indmanstart:inp$indmanend,]
        ylim <- range(c(ylim,BBl_f0[,1:3]))
    }

    ## 2/3
    if("2/3" %in% hcrs){
        res23 <- get.TAC(rep, hcr = "2/3", getFit = TRUE)        
        BBl_23 <- get.par("logBBl",res23, exp = TRUE)[inp$indmanstart:inp$indmanend,]
        ylim <- range(c(ylim,BBl_23[,1:3]))        
    }

    ## dl
    if("dl" %in% hcrs){
        resDL <- get.TAC(rep, hcr = "dl", prob = 0.5, bfrac = 1, getFit = TRUE)
        BBl_dl <- get.par("logBBl",resDL, exp = TRUE)[inp$indmanstart:inp$indmanend,]
        ylim <- range(c(ylim,BBl_dl[,1:3]))        
    }

    ## base plot
    opar <- par(mar=c(5,5,4,2))
    on.exit(par(opar))
    plot(time, BBl[,2], ty='n', ylim = ylim,
         ylab = expression("B/"~B[current]), xlab = "Time")    
    polygon(c(time_nopred,rev(time_nopred)),
            c(BBl_nopred[,1],rev(BBl_nopred[,3])), border = cicol2,
            col=cicol2) 
    lines(time_nopred,BBl_nopred[,2], col="blue")

    ## F = 0    
    if("f0" %in% hcrs){
        polygon(c(timepred,rev(timepred)),
                c(BBl_f0[,1],rev(BBl_f0[,3])), border = cicol3,
                col=cicol3)
        lines(timepred, BBl_f0[,2], col="green")
    }
    ## 2/3
    if("2/3" %in% hcrs){
        polygon(c(timepred,rev(timepred)),
                c(BBl_23[,1],rev(BBl_23[,3])), border = cicol5,
                col=cicol5)
        lines(timepred, BBl_23[,2], col="orange")
    }
    ## dl
    if("dl" %in% hcrs){
        polygon(c(timepred,rev(timepred)),
                c(BBl_dl[,1],rev(BBl_dl[,3])), border = cicol4,
                col=cicol4)
        lines(timepred, BBl_dl[,2], col="red")
    }

    ## refs
    abline(h=1, lty=2)
    abline(v=Tman, lty=2, col="grey70")

    ## legend
    legend("topleft", legend=c("F=0","spict-DL","2/3"),
           col=c("green","red","orange"), lty=1, bty='n',cex=0.9)
