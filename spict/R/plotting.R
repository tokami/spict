...
plotspict.seasonProd <- function(rep, stamp=get.version()){
    ...
    if (!'sderr' %in% names(rep) & ('logphiProd' %in% names(rep$par.fixed) | 'logSdSP' %in% names(rep$par.fixed))){
        jan <- as.POSIXct("2015-01-01 00:00:01 UTC", tz='UTC')
        apr <- jan+(31+28+31)*24*60*60
        jul <- apr+(30+31+30)*24*60*60
        oct <- jul+(31+31+30)*24*60*60
        ...
        if (rep$inp$seasonalProd==1){
            logphiProd <- get.par('logphiProd', rep)
            naflag <- any(is.na(logphiProd[, 1]))            
            inds <- which(colnames(rep$cov.fixed) == 'logphiProd')
            ssf <- get.par('seasonsplinefineProd', rep)
            covmat <- rep$cov.fixed[inds, inds]

            seasonsplineestProd <- get.splineProd(logphiProd[, 2], order=rep$inp$splineorderProd,
                                                  dtfine=rep$inp$dteuler)
            test <- seq(0, 1, length=length(seasonsplineestProd))
            ## yest <- exp(meanlogB + seasonsplineestP)
            yest <- exp(meanlogB + seasonsplineestProd)
            seasonsplinesmooProd <-  ssf[, 2]
            ## sssl <- exp(meanlogB + ssf[, 1])
            sssl <- exp(meanlogB + ssf[, 1])
            ##sssu <- exp(meanlogB + ssf[, 3])
            sssu <- exp(meanlogB + ssf[, 3])
            nsss <- length(seasonsplinesmooProd)
            t <- seq(0, 1, length=nsss)
            ## y <- exp(meanlogB + seasonsplinesmooProd)
            y <- exp(meanlogB + seasonsplinesmooProd)
            ...
            if ("true" %in% names(rep$inp)){
                ylim <- range(ylim,exp(rep$inp$true$SPvec))
            }            

            plot(t, y, typ='n', xaxt='n', xlab='Time of year', ylab='Seasonal spline',
                 main=paste('Seasonal production - spline order:',rep$inp$splineorderProd), ylim=ylim)
        ...
        if(rep$inp$seasonalProd ==2){
            ...
            SPvec <- get.par("SPvec",rep,exp=TRUE)
            y <- rbind(SPvec,SPvec[1,])
            t <- seq(0,1,rep$inp$dteuler)
        ...
        if(rep$inp$seasonalProd ==1){
            lines(test, yest, lwd=1.5, col=4, typ='s')
        }else if(rep$inp$seasonalProd ==2){
            lines(t,y[,2],col=4, lwd=1.5, ty='s')
        }
        
        if ("true" %in% names(rep$inp)){
            if (rep$inp$true$seasonalProd %in% c(1,2)){
                lines(seq(0,1,rep$inp$dteuler),exp(c(rep$inp$true$SPvec,rep$inp$true$SPvec[1])), col="orange", type='s')
            }
        }
