...
#' @name plotspict.dl
#' @title Plot harvest control rules - dl
#' @details Plots harvest control rules
#' @param rep A result report as generated by running fit.spict.
#' @param stamp Stamp plot with this character string.
#' @return Nothing.
#' @export
plotspict.dl <- function(rep, stamp=get.version()){
    ...
    ## F = observed
    plot(time,BBl[,2], ty='n', ylim = c(0,max(as.numeric(BBl[,1:3]))),
         ylab = "B/Bl", xlab = "Time")
    polygon(c(time,rev(time)),
            c(BBl[,1],rev(BBl[,3])), border = cicol2,
            col=cicol2) 
    lines(time,BBl[,2])
    abline(h=1, lty=2)
    abline(v=Tman, lty=2, col="grey70")
    abline(v=Tpred, lty=2, col="grey70")

    txt.stamp(stamp)
    
    if(FALSE){
        ## F = 0
        inpt <- make.ffacvec(rep$inp, 1e-8)
        rep$obj$env$data$ffacvec <- inpt$ffacvec
        rep$obj$env$data$MSEmode <- 2
        rep$obj$retape()
        rep$obj$fn(rep$opt$par)
        sdr <- sdreport(rep$obj)
        BBlF0 <- get.par("logBBl",sdr, exp = TRUE)[inp$indmanstart:inp$indmanend,]


        polygon(c(timepred,rev(timepred)),
                c(BBlF0[,1],rev(BBlF0[,3])), border = NA,
                col=rgb(t(col2rgb("green"))/255, alpha = 0.2))
        lines(timepred, BBlF0[,2], col="green", lty=2)
        ...
        BBlFopt <- get.par("logBBl",fit, exp = TRUE)[inp$indmanstart:inp$indmanend,]

        polygon(c(timepred,rev(timepred)),
                c(BBlFopt[,1],rev(BBlFopt[,3])), border = NA,
                col=rgb(t(col2rgb("blue"))/255, alpha = 0.2))
        lines(timepred, BBlFopt[,2], col="blue", lty=2)
