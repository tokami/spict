...
#'     confidence intervals. By default (CI = 0.95), the 95% confidence
#'     intervals are estimated.
#' @param plot.pred.catch.all.years Plot the predicted catch for years where no
#'     catch observations are available (Default: TRUE).
#'
#' @details
#'
...
plotspict.catch <- function(rep, main='Catch', ylim=NULL, qlegend=TRUE, lcol='blue',
                            ...
                            verbose=TRUE, CI = 0.95, plot.pred.catch.all.years = TRUE){
    check.rep(rep)
    if (!'sderr' %in% names(rep)){
        manflag <- any(names(rep) == "man")
        ylabflag <- is.null(ylab) # If null then not manually specified
...
plot.spictcls <- function(x, stamp=get.version(), verbose=TRUE, CI = 0.95,
                          plot.pred.catch.all.years = TRUE, ...){
    check.rep(x)
    rep <- x
...
#'
#' @export
plot2 <- function(rep, stamp=get.version(), verbose=TRUE, CI = 0.95, plot.pred.catch.all.years = TRUE,
                  ...){
    check.rep(rep)
    logax <- FALSE # Take log of relevant axes? default: FALSE
    inp <- rep$inp
    ...
    if(par == "P"){
        ...
        if(par == "logCpred"){
            ...
            get.annual.catch <- function(rep, par="logCpred", exp=TRUE, CI=0.95){
                ...
                if(plot.pred.catch.all.years){
                    catch <- get.par('logCpredAll', rep, exp=exp, CI = CI)[,1:3]
                    if(exp) catch[catch<0] <- 0
                    time <- unique(round(rep$inp$time[-rep$inp$indpred]))
                }
