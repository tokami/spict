...
#' @details This function requires that you have latex functions installed (pdflatex).
...
#' @param reportfile A \link{connection}, or a character string naming the file to print to. 
#'     If not a connection, \code{make.report} prints to the working directory (default).
...
#' @return Nothing.
#' @export
make.report <- function(rep, reporttitle='', reportfile='report.tex', keep.figurefiles=FALSE, keep.txtfiles=FALSE, keep.texfiles=FALSE){
    ...
    # Summary
    sumname <- 'summaryout.txt'
    summaryoutfile <- file.path(dirname(reportfile),sumname)
